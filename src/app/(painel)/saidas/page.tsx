...
import { buscarGastosDoMes, excluirDivida, excluirSaida } from "./actions";
...
export default async function PaginaSaidas({
  ...
  const mesAnteriorStr = mes === 1 ? `${ano - 1}-12` : `${ano}-${String(mes - 1).padStart(2, "0")}`;
  const mesSeguinteStr = mes === 12 ? `${ano + 1}-01` : `${ano}-${String(mes + 1).padStart(2, "0")}`;
    ...
    <>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/saidas/nova" className="rounded-xl bg-gradient-to-r from-azul-claro to-azul-escuro px-4 py-2 text-sm font-bold text-white shadow hover:opacity-90 transition">
            + Nova saída
          </Link>
        </div>
      </div>
      ...
      <Cartao titulo="Lançamentos">
        {itens.length === 0 ? (
          <EstadoVazio>
            ...
            <Link href="/saidas/nova" className="font-semibold text-azul-texto hover:underline">Adicionar →</Link>
          </EstadoVazio>
        ) : (
          <TabelaTransacoes colunas={["Data", "Descrição", "Categoria", "Parcela", "Por", "Valor", ""]}>
            {itens.map((item) => {
              const acao = item.tipo === "saida"
                ? excluirSaida.bind(null, item.id)
                : excluirDivida.bind(null, item.id);
              return (
                <LinhaTabela key={item.id}>
                  ...
                  <CelulaTabela className="w-8">
                    <BotaoExcluir acao={acao} mensagem={`Excluir "${item.descricao}"?`} />
                  </CelulaTabela>
                </LinhaTabela>
              );
            })}
